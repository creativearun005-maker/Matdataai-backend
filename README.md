...
**Convert scanned Hindi voter-list PDFs into structured, filterable Excel data that can be segregated ward-wise, house-wise, near-zero cost according to the candidate's need.**
...
**Stage 1 — Page classification.** Each PDF mixes cover pages(not the voters), polling-station-photo pages(not the voters), the voter grid(the needed voters), and summary pages(not the voters). Rather than assuming a fixed page layout, we detect the 3-column voter-grid structure directly (contour/column analysis) — this generalizes across documents with a different number of intro/summary pages, and correctly handles partial last-pages.
...
- **Upcoming Plan:** Fine-tune OCR model on this document's specific font/print-style (own-corpus training, removing external API dependency for numeric fields), complete handwritten-field pipeline, EPIC-format validation.
